...
Array.from(order).forEach(function(element){
    orderList.add({
        sku_order: element.sku_order,
        customer_name: element.user.name,
        date: str_dt(element.created_at),
        amount: element.total_amount,
        payment:element.payment.payments_method,
        staff: element.role.name,
        status: isStatus(element.status_order)
    });
    orderList.sort('id', { order: "desc" });
    refreshCallbacks();
});
orderList.remove("sku_order", `<a href="apps-ecommerce-order-details" class="fw-medium link-primary">#VZ2101</a>`);
