                        ...
                        <div class="input-group me-3">
                            <select id="status_order_filter" class="form-select" name="status_order">
                                ...
                                <option value="confirmed"
                                    {{ request('status_order') == 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                                <option value="processing"
                                    {{ request('status_order') == 'processing' ? 'selected' : '' }}>Processing</option>
                                <option value="shipping" {{ request('status_order') == 'shipping' ? 'selected' : '' }}>
                                    Shipping</option>
                                <option value="delivered"
                                    {{ request('status_order') == 'delivered' ? 'selected' : '' }}>Delivered</option>
            ...
            <div class="card-body">
                <table id="example" class="table table-bordered dt-responsive nowrap table-striped align-middle"
                    ...
                    <tbody>
                        ...
                        @include('admin.orders._order_table', ['orders' => $data])

                    </tbody>

                </table>
                <div class="paginations mt-3 d-flex justify-content-end">
                    {{ $data->links() }}
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    $('#status_order_filter').change(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('.pagination').html(response.pagination); // Hiển thị phân trang
            },
            error: function(xhr) {
    ...
    $(document).on('click', '.pagination a', function(e) {
        e.preventDefault();
        let url = $(this).attr('href'); // Lấy URL từ nút phân trang
